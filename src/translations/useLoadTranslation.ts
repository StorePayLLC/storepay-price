...
import { AVAILABLE_LOCALES, DEFAULT_LOCALE, isAcceptedLocale, selectFirstAcceptedLocale } from '@/config/internalization';
...
const useLoadTranslation = (i18nPropsNamespace: I18nPropsNamespace, locale: AVAILABLE_LOCALES) => {
  ...
  useMemo(() => {
    ...
    activateLocale(globalI18n, isAcceptedLocale(locale) ? locale : DEFAULT_LOCALE, i18nPropsNamespace.initialMessages);
  }, []);
  ...
  useEffect(() => {
    ...
    activateLocale(globalI18n, isAcceptedLocale(locale) ? locale : DEFAULT_LOCALE, i18nPropsNamespace.initialMessages);
  }, [expectedLocale, i18nPropsNamespace, locale]);
};
